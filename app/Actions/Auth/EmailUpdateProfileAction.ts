...
export default class UpdateProfileAction {
    public async handle(user, data) {
          ...
          await user.merge({ confirmation_digest: digest, email: data.email, confirmed_at: null, confirmation_sent_at: DateTime.utc() }).save()
    
          new UpdateEmailConfirmationMailer(user.auth, token).sendLater()
